...
def _smart_imread(img, flag=cv2.IMREAD_UNCHANGED):
    if img.endswith(".gif"):
        img = Image.open(img)
        img = img.convert("RGB")
...
def _load_model(model_repo: str, model_filename: str) -> rt.InferenceSession:
...
class Wd14Tagger:
    def __init__(self, model_name: str, general_treshold = 0.1, character_treshold = 0.1):
        self.labels = _load_labels(model_name, "selected_tags.csv")
        self.model = _load_model(model_name, "model.onnx")
        self.name = model_name
        self.general_treshold = general_treshold
        self.character_treshold = character_treshold
    def tags(self, image_path):
        image = Image.open(image_path).convert("RGB")
        try:
            return _predict(image, self.model, self.general_treshold, self.character_treshold, self.labels[0], self.labels[1], self.labels[2], self.labels[3])
